...
take_snapshots() {
  for EMU in "${EMULATORS_ARRAY[@]}"; do
    ...
    $ANDROID_HOME/emulator/emulator -avd $EMU -wipe-data -no-boot-anim -no-audio -no-snapshot-load &
    >/dev/null 2>&1
    until $ANDROID_HOME/platform-tools/adb -s emulator-5554 shell dumpsys window | grep -E 'mFocusedApp' | grep -q 'ActivityRecord' >/dev/null 2>&1; do
    ...
    echo "Take snapshot of $EMU"
    {
      ...
      echo "avd snapshot save clean"
      sleep 30
    } | telnet localhost 5554 >/dev/null 2>&1
